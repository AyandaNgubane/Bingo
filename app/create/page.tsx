...
import { GameMode, WinPattern, CallingMode, WIN_PATTERN_LABEL } from "@/lib/types";
...
export default function CreateGame() {
  ...
  const [callingMode, setCallingMode] = useState<CallingMode>("auto");
  ...
  function togglePack(packId: string) {
    ...
    const existingLines = contentText
      .split("\n")
      .map((l) => l.trim())
      .filter(Boolean);

    if (isActive) {
      // Remove this pack's items (only ones that exactly match, so manual edits are kept)
      const toRemove = new Set(pack.items);
      setContentText(existingLines.filter((l) => !toRemove.has(l)).join("\n"));
    } else {
      const existingSet = new Set(existingLines);
      const additions = pack.items.filter((item) => !existingSet.has(item));
      setContentText([...existingLines, ...additions].join("\n"));
    }

    setActivePacks((prev) => {
      const next = new Set(prev);
      isActive ? next.delete(packId) : next.add(packId);
      return next;
    });
  }
  ...
  return (
    <main className="min-h-screen px-6 py-12">
      <div className="max-w-xl mx-auto">
        ...
        <section className="mb-8">
          <h2 className="font-display font-bold text-card mb-1">How items get called</h2>
          <p className="text-muted text-sm mb-3">
            {mode === "custom"
              ? "Custom content works well as a song round: play each track yourself, and players mark it by ear."
              : "Classic numbers are usually called out loud by the host."}
          </p>
          <div className="grid grid-cols-2 gap-3">
            <button
            ...
            >
              <div className="font-display font-bold">Auto-reveal</div>
              <div className="text-sm mt-1">Players see each call and their board highlights it</div>
            </button>
            <button
            ...
            >
              <div className="font-display font-bold">Manual / by ear</div>
              <div className="text-sm mt-1">Only you see what's called — players tick it themselves</div>
            </button>
          </div>
        </section>
        ...
        {mode === "custom" && (
          <section className="mb-6">
            <h2 className="font-display font-bold text-card mb-1">Christian & Gospel content packs</h2>
            <p className="text-muted text-sm mb-3">
              Pentecostal-aligned lists you can drop straight in. Tap to add or remove — mix
              as many as you like.
            </p>
                ...
                return (
                  <button
                    ...
                    className={`text-left rounded-lg border-2 px-3 py-2.5 transition-colors ${
                      active
                        ? "border-teal bg-teal/15 text-card"
                        : "border-card/15 text-muted hover:border-card/30"
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-display font-bold text-sm">{pack.label}</span>
                      <span className="text-xs">{active ? "Added ✓" : `+${pack.items.length}`}</span>
                    </div>
                    <div className="text-xs mt-0.5">{pack.description}</div>
                  </button>
                );
              })}
            </div>
        ...
        {mode === "custom" && (
          <section className="mb-8">
            ...
            <p className="text-muted text-sm mb-3">
              One item per line — edit freely after adding a pack. Each board gets a random,
              unique arrangement. You need at least {needed} for a {gridSize}×{gridSize} board.
            </p>
            <textarea
              value={contentText}
